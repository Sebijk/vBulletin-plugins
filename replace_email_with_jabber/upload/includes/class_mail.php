...
class vB_SmtpMail extends vB_Mail
{
	...
	*/
	function vB_SmtpMail(&$registry)
	{
		...
		$this->jabberuser = explode("@", $this->registry->options['smtp_user']);
	...
	*/
	...
	function send()
	{
		...
		require_once(DIR."/includes/XMPPHP/XMPP.php");
		$this->smtpSocket = new XMPPHP_XMPP($this->smtpHost, $this->smtpPort, $this->jabberuser[0], $this->smtpPass, "vBulletin", $jabber_user[1], $printlog=false, $loglevel=XMPPHP_Log::LEVEL_INFO);
		$this->smtpSocket->connect();
    $this->smtpSocket->processUntil('session_start');
    
    // If you use UTF-8 as Standard Charset, you can delete this lines. This is only needed for ISO-8859-1 compatibility.
    $this->message = utf8_encode($this->message);
    $this->subject = utf8_encode($this->subject);
    // End of ISO-8859-1 compatibility.

			$this->smtpSocket->message($this->toemail, $this->message, "chat", $this->subject);
